...
prompt({
  message: "Ingresa el anime a buscar en https://animeflv.video/",
  name: "name",
}).then(async (res) => {
  ...
  listOfEpisodes = listOfEpisodes.map((ep, i, arr) => {
    return {
      name: ep.name.replace(arr.length - i, i + 1),
      value: ep.value,
      short: ep.short.replace(arr.length - i, i + 1),
    };
  });
  const { episode } = await prompt({
    message: "¿Que capitulo quieres ver?",
    choices: listOfEpisodes,
    name: "episode",
    type: "list",
  });
